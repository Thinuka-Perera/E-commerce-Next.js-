...
const ProductCard = ({product}:{product:ProductType}) => {
  ...
  const handleAddToCart = () => {
     ...
     toast.success("Success to added product to cart")
  };
            ...
            <div className='flex flex-col gap-1'>
                ...
                <div className='flex items-center gap-2'>
                    {product.colors.map(color => (
                        <div className={`cursor-pointer border-1 ${productTypes.color === color ? 'border-gray-950':'border-gray-200'} rounded-full p-[1.2px] `}
                        key={color} onClick={() => handleProductType({type:"color",value:color})}>
                            <div className='w-[14px] h-[14px] rounded-full border border-gray-400' style={{ backgroundColor:color }} />
                        </div>
                    ))}
                </div>
            </div>
